...
const buttonVariants = cva(
  "relative inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-medium select-none [transition:transform_0.2s_cubic-bezier(0.34,1.56,0.64,1),background-color_0.25s_ease,border-color_0.25s_ease,color_0.25s_ease,box-shadow_0.25s_ease,opacity_0.25s_ease] will-change-transform active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 disabled:active:scale-100 motion-reduce:transition-none motion-reduce:active:scale-100 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        // Primary call-to-action (login submit, error retry…)
        default:
          "bg-primary text-primary-foreground shadow-sm hover:brightness-95 hover:shadow-md",
        // Bordered secondary action
        outline:
          "border border-border bg-transparent hover:bg-primary-soft hover:text-primary",
        // Solid neutral (uses theme accent)
        secondary:
          "bg-accent text-background shadow-sm hover:opacity-90 hover:shadow-md",
        // Destructive (delete)
        destructive:
          "border border-destructive text-destructive hover:bg-red-500/10",
