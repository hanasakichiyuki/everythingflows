  ...
  return (
    <article className={`anim-fade-up anim-delay-${Math.min(index + 1, 6)} group`}>
      <Link
        ...
        className="block rounded-2xl border border-surface-border bg-surface px-5 py-6 shadow-sm transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-[0_18px_42px_-32px_rgba(25,74,91,0.45)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:px-7 sm:py-7"
      >
        <div className="flex items-start justify-between gap-5">
          <div className="min-w-0">
            ...
            <h2 className="mt-3 font-serif text-xl font-semibold tracking-tight text-foreground transition-colors group-hover:text-primary sm:text-[1.65rem]">{post.title}</h2>
            <p className="mt-2 line-clamp-2 max-w-5xl text-sm leading-6 text-muted sm:text-[15px] sm:leading-7">{post.description}</p>
          </div>
